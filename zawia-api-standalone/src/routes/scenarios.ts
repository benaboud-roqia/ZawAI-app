...
scenariosRouter.post("/scenarios/generate", async (req, res) => {
  const { niche = "Lifestyle", topic = "", duration = 30, platform = "TikTok" } = req.body ?? {};
...
scenariosRouter.post("/schedule/optimal-times", async (req, res) => {
  const { platforms = ["instagram", "tiktok"], timezone = "Africa/Algiers" } = req.body ?? {};
